<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0a0a0a,50:1a1a2e,100:16213e&height=180&section=header&text=Akshaj%20Kurra%20Satishkumar&fontSize=46&fontColor=e2e8f0&fontAlignY=40&desc=Backend%20%7C%20AI%20Systems%20%7C%20Distributed%Systems&descSize=16&descAlignY=62&descColor=94a3b8" />
</p>

<p align="center">
  <a href="https://akshajks.com">
    <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=16&pause=1000&color=38BDF8&center=true&vCenter=true&width=620&lines=LLM+Infrastructure+%26+RAG+Pipelines;Distributed+Systems+%40+Scale;MS+CS+%40+UIC+%E2%80%94+GPA+3.9%2F4.0;Graduating+May+2026+%E2%80%94+Open+to+New+Grad+Roles" alt="Typing SVG" />
  </a>
</p>

<br/>

<p align="center">
  <a href="https://linkedin.com/in/akshajks">
    <img src="https://skillicons.dev/icons?i=linkedin" height="40" title="LinkedIn"/>
  </a>
  &nbsp;
  <a href="https://akshajks.com">
    <img src="https://skillicons.dev/icons?i=vercel" height="40" title="Portfolio"/>
  </a>
  &nbsp;
  <a href="mailto:akurr@uic.edu">
    <img src="https://skillicons.dev/icons?i=gmail" height="40" title="Email"/>
  </a>
  &nbsp;
  <a href="https://github.com/realAkshaj">
    <img src="https://skillicons.dev/icons?i=github" height="40" title="GitHub"/>
  </a>
</p>

---

## About Me

I'm a CS grad student at the **University of Illinois Chicago** (GPA 3.9) building production-grade AI systems. My work spans LLM infrastructure, distributed backend engineering, and data analytics. I care about systems that are fast, reliable, and actually ship.

Before grad school, I built backend infrastructure at a startup processing **50K+ daily transactions** with 99.9% uptime. Now focused on full-stack engineering, LLM agents, RAG systems, and scalable ML serving.

---

## Featured Projects

| Project | Description | Stack |
|--------|-------------|-------|
| [**Distributed Order Orchestration System**](https://github.com/realAkshaj/) | Saga-orchestrated microservices with circuit breakers, idempotency, and RabbitMQ event bus | Java, Spring Boot, RabbitMQ, Redis, Docker |
| [**LLM Conversational Agent**](https://github.com/realAkshaj/LLM-Microservice) ([Video](https://youtu.be/zWOe9C6i96g)) | Hybrid cloud/local chat integrating AWS Bedrock & Ollama via Akka HTTP microservices | Scala, Akka HTTP, AWS Bedrock, Ollama, Docker |
| [**AI CMS Platform**](https://github.com/realAkshaj/ai-cms-platform) ([Live](https://ai-cms-platform-web.vercel.app/)) | Full-stack AI-powered CMS with Gemini integration, multi-tenant auth, and analytics dashboard | TypeScript, Next.js, Prisma, PostgreSQL, Docker |
| [**Graph-Based Skill Prediction Engine**](https://github.com/realAkshaj/) | Inductive GNN on O\*NET workforce data for skill recommendation and occupation similarity | Python, PyTorch, GraphSAGE, O\*NET |
| [**Portfolio**](https://github.com/realAkshaj/portfolio) ([Live](https://akshajks.com)) | OS-style personal site with desktop metaphor UI, draggable windows & animated transitions | Next.js 14, TypeScript, Tailwind, Framer Motion |

---

## Tech Stack

**Languages**

<p>
  <img src="https://skillicons.dev/icons?i=python,java,ts,js,golang,cpp,scala" />
</p>

**AI / ML**

<p>
  <img src="https://skillicons.dev/icons?i=pytorch,tensorflow,scikitlearn" />
  &nbsp;
  <img src="https://iconic-api.onrender.com/dark/matplotlib" width="45" title="Matplotlib"/>
  <img src="https://iconic-api.onrender.com/dark/chatgpt" width="45" title="OpenAI"/>
  &nbsp;
  <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white" />
  &nbsp;
  <img src="https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logo=huggingface&logoColor=black" />
</p>

**Backend & Systems**

<p>
  <img src="https://skillicons.dev/icons?i=spring,fastapi,nodejs,graphql,redis,rabbitmq,kafka" />
</p>

**Data**

<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql,mongodb,cassandra,dynamodb" />
</p>

**Cloud & Infra**

<p>
  <img src="https://skillicons.dev/icons?i=aws,docker,kubernetes,terraform,githubactions,nginx" />
</p>

**Frontend**

<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,ts,prisma,vite" />
</p>

---

## GitHub Stats

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=realAkshaj&theme=github_dark" width="100%" />
</p>
<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=realAkshaj&theme=github_dark" width="32%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=realAkshaj&theme=github_dark" width="32%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=realAkshaj&theme=github_dark" width="32%" />
</p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:16213e,50:1a1a2e,100:0a0a0a&height=100&section=footer" />
</p>
